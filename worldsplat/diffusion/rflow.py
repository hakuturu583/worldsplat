"""Rectified Flow scheduler for training and sampling."""
...
def mean_flat(tensor, mask=None):
    """Take the mean over all non-batch dimensions, optionally with a frame mask."""
    if mask is None:
...
def timestep_transform(
    ...
    """Apply resolution- and duration-aware timestep scaling."""
...
class RFlowScheduler:
    """Low-level rectified flow scheduler that handles noise addition and loss computation."""
    ...
    def __init__(
        ...
        if sample_method == "logit-normal":
            ...
            self.sample_t = lambda x: self.distribution.sample((x.shape[0] // 6,))[:, 0].to(x.device)

        self.use_timestep_transform = use_timestep_transform
        self.transform_scale = transform_scale

    def training_losses(self, model, x_start, model_kwargs=None, noise=None, mask=None, weights=None, t=None):
        """Compute rectified flow training losses (velocity matching)."""
        num_frames = x_start.shape[0] if x_start.shape[0] < 8 else 6
        if t is None:
            if self.use_discrete_timesteps:
                t = torch.randint(0, self.num_timesteps, (x_start.shape[0] // num_frames,), device=x_start.device)
            elif self.sample_method == "uniform":
                t = torch.rand((x_start.shape[0] // num_frames,), device=x_start.device) * self.num_timesteps
    ...
    def add_noise(self, original_samples, noise, timesteps):
        """Add noise via linear interpolation (rectified flow formulation)."""
        timepoints = timesteps.float() / self.num_timesteps
        timepoints = 1 - timepoints  # [1, 1/num_timesteps]
...
class RFLOW:
    """Rectified Flow wrapper providing training_losses() and sample() with classifier-free guidance."""
    ...
    def sample(
        ...
        """Generate samples using rectified flow ODE with classifier-free guidance."""
    ...
    def training_losses(self, model, x_start, model_kwargs=None, noise=None, mask=None, weights=None, t=None):
        """Compute rectified flow training losses (delegates to scheduler)."""
